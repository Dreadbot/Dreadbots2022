...
def similar_triangles_calculation(u, v, K, R):
    ...
    world_vector_world = np.array([-x, -y, z])
...
def reverse_point(world_vector, K, R, round=False):
    ...
    world_vector_world = world_vector_camera*math.cos(angle) + np.cross(unit, world_vector_camera)*math.sin(
        angle) + unit*np.dot(unit, world_vector_camera)*(1-math.cos(angle))
...
def center_distance(point, K, angle_offset=14.5, height=81.5):
    fy, cy = K[1, 1], K[1, 2]
    ...
    vertical_angle_radians = math.atan(
        dy/fy) + angle_offset_radians  # Vertical Angle
...
def geometric_true_center(p1, p2, draw_target=None):  # Rework later for u,v,K
    ...
    fx, fy = K[0, 0], K[1, 1]
    camera_cx, camera_cy = K[0, 2], K[1, 2]
    ...
    cv2.line(draw_target, (int(target_px), 0),
             (int(target_px), 480), (0, 255, 0))
    ...
    if draw_target is not None:
        ...
        for p in range(len(draw_pts)):
            ...
            cv2.circle(draw_target, draw_pts[p],
                       2, (200, 50, 50), thickness=-1)
        ...
        major_axis_left = int(
            math.sqrt((left[0] - center[0])**2 + (left[1] - center[1])**2))
        major_axis_right = int(
            math.sqrt((right[0] - center[0])**2 + (right[1] - center[1])**2))

        minor_axis_far = int(
            math.sqrt((far[0] - center[0])**2 + (far[1] - center[1])**2))
        minor_axis_close = int(
            math.sqrt((close[0] - center[0])**2 + (close[1] - center[1])**2))
        ...
        vertical_angle = - \
            math.degrees(
                math.atan((close[0]-center[0]) / (close[1]-center[1])))
        ...
        vertical_points = np.array([
            rotate_2d(np.array([cx, cy-minor_axis]),
                      vertical_angle, (cx, cy), round=True),
            rotate_2d(np.array([cx, cy+minor_axis]),
                      vertical_angle, (cx, cy), round=True)
        ]).astype(int)

        vp1, vp2 = (vertical_points[0, 0], vertical_points[0, 1]
                    ), (vertical_points[1, 0], vertical_points[1, 1])

        cv2.line(draw_target, hp1, hp2, (0, 255, 0))
        cv2.line(draw_target, vp1, vp2, (0, 255, 0))

        cv2.ellipse(draw_target, center, (major_axis, minor_axis),
                    0, 0, 360, color, thickness=thickness)
        cv2.circle(draw_target, center, 3, (0, 255, 0), thickness=-1)

    return(True, horizontal_angle, distance)


def calibrate_intrinsic(cam_id, outputStream, table: NetworkTable, img_count=20):
